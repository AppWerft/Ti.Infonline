...
static void Infonline_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Infonline.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::InfonlineBindings::lookupGeneratedInit(
...
static void Infonline_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Infonline_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Infonline_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::InfonlineBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry InfonlineBinding = {
	"ti.infonline",
	Infonline_init,
	Infonline_dispose
};
...
Java_ti_infonline_InfonlineBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.infonline", &InfonlineBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::InfonlineBindings::lookupGeneratedInit));
}
